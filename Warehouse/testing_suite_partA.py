...
def truncate_output( s, max_len = 6000 ):
    if len(s) > max_len:
...
class Submission:
    ...
    def execute_student_plan(self, warehouse, boxes_todo):
        ...
        try:
            student_planner = DeliveryPlanner_PartA(copy.copy(warehouse), copy.copy(boxes_todo))
...
class PartATestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if studentExc:
            ...
            self.results.append( "exception on import: %s" % str(e) )
            raise studentExc
