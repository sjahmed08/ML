...
def truncate_output( s, max_len = 20000000 ):
    if len(s) > max_len:
...
class Submission:
    ...
    def execute_student_plan(self, warehouse, box_todo, max_distance, max_steering):
        ...
        try:
            ...
            student_planner = DeliveryPlanner_PartB(copy.copy(warehouse), copy.copy(box_todo),
                                                    max_distance, max_steering)

            action_list = student_planner.plan_delivery()
            self.action_list_log = action_list
            if DRAW:
                drawMoves.drawWH2(exec_count, warehouse, box_todo, action_list)
    ...
    def _attempt_lift(self, box_id):
        ...
        try:
            ...
            if is_legal_lift:
                ...
                if VERBOSE_FLAG:
                    self._log(
                        "*** successfully lifted box: box_is_adjacent = {}, robot_has_box = {}. box {} at {}, robot at {}, clear distance: {} ".format(
                            box_is_adjacent,
                            robot_has_box, box_id, box_position, (self.robot.x, self.robot.y), robot.compute_distance((self.robot.x, self.robot.y), box_position)))
            else:
                if VERBOSE_FLAG:
                    self._log("*** Could not lift box: box_is_adjacent = {}, robot_has_box = {}. box {} at {}, robot at {}, clear distance: {} ".format(
                            box_is_adjacent,
                            robot_has_box, box_id, box_position, (self.robot.x, self.robot.y), robot.compute_distance((self.robot.x, self.robot.y), box_position)))
    ...
    def _attempt_down(self, x, y):
        ...
        try:
            ...
            if is_legal_down:
                ...
                if VERBOSE_FLAG:
                    self._log(
                        "*** successfull drop off: dropoff at = {}, robot at {} , is_adjest: {}, is_open: {}, is_within_wh: {}, distance: {}".format(
                            (x, y), (self.robot.x, self.robot.y), destination_is_adjacent, destination_is_open,
                            destination_is_within_warehouse, robot.compute_distance(
                    (self.robot.x, self.robot.y), (x,y))))
            else:
                if VERBOSE_FLAG:
                    self._log(
                        "*** Could not drop box: dropoff at = {}, robot at {} , is_adjest: {}, is_open: {}, is_within_wh: {}, distance: {}".format(
                            (x, y), (self.robot.x, self.robot.y), destination_is_adjacent, destination_is_open,
                            destination_is_within_warehouse, robot.compute_distance(
                    (self.robot.x, self.robot.y), (x,y))))
