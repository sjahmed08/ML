...
def draw_tl_center(image_in, center, state):
    ...
    """
    x, y = center
    cv2.circle(image_in,(x, y),1,(255,0,0),3)
    text="({}, '{}')".format((x, y), state)
    # print text
    cv2.putText(image_in, text, (x+30, y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255,0,0), 2)

#     cv2.imshow('Traffic signs',image_in)
#     cv2.waitKey(0)
    return image_in
...
def mark_traffic_signs(image_in, signs_dict):
    ...
    for key, value in signs_dict.items():
        xy = value
        cv2.circle(image_in,(xy[0], xy[1]),1,(255,0,0),3)
        ...
        if other == False:
            ...
            disp_y = -25
            other = False
        print ("text", text, disp)
        cv2.putText(image_in, text, (xy[0]-100, xy[1]+disp_y), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255,0,0), 1)
